...
FILES_TO_INLINE = {
    ...
    "app/utils/theme.py": "app/utils/theme.py",
    "app/utils/filters.py": "app/utils/filters.py",
    ...
    "app/pages/1_Signal_Triage.py": "app/pages/1_Signal_Triage.py",
    "app/pages/2_Trends_and_Topics.py": "app/pages/2_Trends_and_Topics.py",
...
def download_stlite():
    ...
    try:
        subprocess.run(
            [npm_cmd, "pack", f"{STLITE_PACKAGE}@{STLITE_VERSION}", "--pack-destination", str(tmp_dir)],
            check=True,
            capture_output=True,
        )
...
def build_site():
    ...
    html_parts = [
        ...
        "  <title>Community Pulse — Pure Intelligence Console</title>",
        "  <style>",
        ...
        "<body>",
        "  <div id=\"root\">",
        "    <p style=\"padding:40px;text-align:center;color:#9A9A9A;font-family:monospace;\">",
        "      Loading Community Pulse...<br>",
        "      <small>(first load may take 30-60 seconds while Stlite initializes)</small>",
        "    </p>",
        "  </div>",
        "  <script type=\"module\">",
        ...
        "      try {",
        "        root.innerHTML = \"<p style='padding:20px;text-align:center;color:#9A9A9A;font-family:monospace;'>Loading Stlite runtime...</p>\";",
        "        const stlite = await import(\"./stlite/stlite.js\");",
        "        root.innerHTML = \"<p style='padding:20px;text-align:center;color:#9A9A9A;font-family:monospace;'>Mounting Streamlit app...</p>\";",
        "        await stlite.mount({",
        ...
        "          streamlitConfig: {",
        "            \"theme.base\": \"dark\",",
        "            \"theme.primaryColor\": \"#FE5000\",",
        "            \"theme.backgroundColor\": \"#0A0A0A\",",
        "            \"theme.secondaryBackgroundColor\": \"#161616\",",
        ...
        "      } catch (e) {",
        "        root.innerHTML =",
        "          \"<pre style='padding:20px;color:#FE5000;white-space:pre-wrap;'>\" +",
        "          \"Stlite failed: \" + (e.message || String(e)) + \"</pre>\" +",
        "          \"<pre style='padding:20px;color:#9A9A9A;white-space:pre-wrap;font-size:12px;'>\" +",
        "          \"Check browser console (F12) for details.\\n\" +",
        "          \"If this persists, the Stlite runtime may not be loading correctly from /stlite/</pre>\";",
        "        console.error(\"Stlite init error:\", e);",
        "      }",
        "    }",
        "    initStlite();",
        "  </script>",
